...
var Module = {
	...
	UE4_indexedDBVersion: 201905191611, // Bump this number to invalidate existing IDB storages in browsers.
};
...
$(document).ready(function() {
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://media.githubusercontent.com/media/comountainclimber/escape_room/master/BuildingEscape.wasm'), 'arraybuffer').then(function(wasmBytes) {
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://media.githubusercontent.com/media/comountainclimber/escape_room/master/BuildingEscape.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://media.githubusercontent.com/media/comountainclimber/escape_room/master/BuildingEscape.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
